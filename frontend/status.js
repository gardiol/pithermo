          ...
          ConfirmDialog )// Dijit
{
    sts = {
        ...
        confirm: function(msg,ok,cmd){
            ...
            putRequest("cgi-bin/command-cgi", cmd, 
        ...
        saveTemp: function(){	
            ...
            putRequest("cgi-bin/command-cgi",c,function(result){sts.update()},function(err){alert("Command error: " + err );});
        },
        toggleSmart: function(){ 
            ...
            dialog.set("buttonCancel", "Annulla");
            dialog.on("execute", function(){
                putRequest("cgi-bin/command-cgi", smart_temp ? "smart-temp-off" : "smart-temp-on", 
                    function(result){sts.update();},
                    function(err){alert("Command error: " + err );});
            });
            dialog.show();
        ...
        saveOffTime: function(){
            ...
            putRequest("cgi-bin/command-cgi","set-mot "+v,function(result){sts.update()},function(err){alert("Command error: " + err );});
        },
        disableOffTime: function(){
            putRequest("cgi-bin/command-cgi","set-mot 0",function(result){sts.update()},function(err){alert("Command error: " + err );});
        },
        setOffTime: function(){
        ...
        update: function(){
            ...
            getRequest("cgi-bin/status-cgi",
				...
				dclass.remove("program-area", "hidden");
				...
				dclass.remove("program-area", "hidden");
            			...
            			status_external.set("disabled", true );
                                ...
                                min_hyst.set("disabled", true);
                                max_hyst.set("disabled", true);
				dclass.add("program-area", "hidden");
			    }
