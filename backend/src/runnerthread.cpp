...
RunnerThread::RunnerThread(ConfigFile *config,
    ...
    PithermoThread("Runner", 1000 * 1000 ),
...

{
    ...
    if ( !_config->isEmpty() )
    {
        std::string config_mode = PithermoUtils::string_tolower(_config->getValue( "mode" ));
        ...
        _min_temp = static_cast<float>(PithermoUtils::string_tof( _config->getValue( "min_temp" ) ) );
        _max_temp = static_cast<float>(PithermoUtils::string_tof( _config->getValue( "max_temp" ) ) );
        ...
        if ( _config->hasValue( "pellet_startup_delay" ) )
            _pellet_startup_delay = static_cast<uint64_t>(PithermoUtils::string_toi( _config->getValue( "pellet_startup_delay" ) ) );
        _temp_correction = static_cast<float>(PithermoUtils::string_tof( _config->getValue( "temp_correction" ) ) );
...
{
    // Commands to read if read&write pointers do not match
    SharedStatus* shared = (SharedStatus*)_shared_status.getWritePtr();
    while ( shared->command_queue_write_ptr != shared->command_queue_read_ptr )
    {
        Command* cmd = new Command( shared->command_queue[ shared->command_queue_read_ptr ].command,
                                   shared->command_queue[ shared->command_queue_read_ptr ].payload,
                                   std::string( shared->command_queue[ shared->command_queue_read_ptr ].payload_string, SharedStatusProgramSize )
                                   );
        if ( _debug_updates )
            _logger->logDebug("Appending command: " + PithermoUtils::tostring( cmd->command() ) );
        _commands_list.push_back( cmd );

        shared->command_queue_read_ptr++;
        if ( shared->command_queue_read_ptr >= SharedStatusCommandQueueSize )
            shared->command_queue_read_ptr = 0;
    }
    ...
    while ( _commands_list.size() > 0 )
    {
        ...
        {
            std::vector<std::string> tokens = PithermoUtils::string_split( cmd->getParamString(), ":" );
            if ( tokens.size() == 3 )
            {
                uint32_t template_no = static_cast<uint32_t>(PithermoUtils::string_toi( tokens[0] ));
        ...
        case Command::SET_MANUAL_OFF_TIME:
            ...
            if ( _current_mode == MANUAL_MODE )
            {
                uint64_t tmp_dmot = cmd->getParam();
                if ( tmp_dmot > 0 )
                {
        ...
        {
            _logger->logDebug("New MIN TEMP received: " + PithermoUtils::dtostring( cmd->getParam() ) );
            float tmp_temp = cmd->getParam();
            if ( (fabs(tmp_temp-_min_temp) > 0.05f) && (tmp_temp < _max_temp) )
            {
                _logger->logDebug("Changed min temp to " + PithermoUtils::dtostring( cmd->getParam() ) );
        ...
        {
            _logger->logDebug("New HYSTERESIS_MAX received: " + PithermoUtils::dtostring( cmd->getParam() ) );
            float tmp_hyst = cmd->getParam();
            if ( tmp_hyst >= 0.1f )
            {
                _logger->logDebug("Changed hysteresis max to " + PithermoUtils::dtostring( cmd->getParam() ) );
        ...
        {
            _logger->logDebug("New HYSTERESIS_MIN received: " + PithermoUtils::dtostring( cmd->getParam() ) );
            float tmp_hyst = cmd->getParam();
            if ( tmp_hyst >= 0.1f )
            {
                _logger->logDebug("Changed hysteresis min to " + PithermoUtils::dtostring( cmd->getParam() ) );
                _logger->logEvent( LogItem::HYST_UPDATE );
        ...
        {
            _logger->logDebug("New MAX TEMP received: " + PithermoUtils::dtostring( cmd->getParam() ) );
            float tmp_temp = cmd->getParam();
            if ( ( fabs(tmp_temp-_max_temp) > 0.05f) && (tmp_temp > _min_temp) )
            {
                _logger->logDebug("Changed max temp to " + PithermoUtils::dtostring( cmd->getParam() ) );
        ...
        case Command::PROGRAM:
            _logger->logDebug("New program received: " + cmd->getParamString() );
            if ( _program.change( cmd->getParamString() ) )
            {
...
bool RunnerThread::scheduledRun(uint64_t, uint64_t)
{
    ...
    uint64_t current_time = PithermoTimer::getTimeEpoc();
    if ( (_last_time+60) <= current_time )
    {
        if ( _debug_updates )
            _logger->logDebug("Time interval (" + PithermoUtils::utostring(current_time) +" - " + PithermoUtils::utostring(_last_time) + " >= 60)");
        ...
        if ( _sensor_success_reads > 0 )
        {
            ...
            if ( _mqtt != NULL )
            {
                ...
                data += PithermoUtils::ftostring( _temp_sensor->getTemp() );
                data += ", \"humidity\": ";
                data += PithermoUtils::ftostring( _temp_sensor->getHumidity() );
                data += " }";
...
void RunnerThread::_saveConfig()
{
    ...
    _config->setValue( "min_temp", PithermoUtils::ftostring( _min_temp ) );
    _config->setValue( "max_temp", PithermoUtils::ftostring( _max_temp ) );
    _config->setValue( "temp_correction", PithermoUtils::ftostring( _temp_correction ) );
    _config->setValue( "pellet_startup_delay", PithermoUtils::utostring(_pellet_startup_delay) );
    _config->setValue( "hysteresis_max", PithermoUtils::ftostring( _hysteresis_max ) );
    _config->setValue( "hysteresis_min", PithermoUtils::ftostring( _hysteresis_min ) );
    _config->setValue( "excessive_overtemp_ts", PithermoUtils::ftostring( _excessive_overtemp_threshold ) );

    ConfigFile* prog_section = _config->getSection( "program" );
...
{
    SharedStatus* status = (SharedStatus*)_shared_status.getWritePtr();
    status->marker = SharedStatusMarker;
    status->last_update_stamp = PithermoTimer::getCurrentTime();
