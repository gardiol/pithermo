    ...
    if ( _data != NULL )
    {
        if ( isConnected() )
        {
            if ( int ret = mosquitto_publish( (struct mosquitto *)_data,
                ...
                _logger->logDebug( "MQTT publish failed for topic '" + topic + "' [" + PithermoUtils::tostring( ret ) + "]" );
        }
        else
...
void MQTT_Interface::_subscribe_topic(const std::string &topic)
{
    if ( _connected )
    {
        if ( int ret = mosquitto_subscribe( (struct mosquitto *)_data, NULL, topic.c_str(), 0 ) != MOSQ_ERR_SUCCESS )
            _logger->logDebug( "MQTT subscribe failed for topic '" + topic + "' [" + PithermoUtils::tostring( ret ) + "]" );
        else
...
void MQTT_Interface::_connect_callback(mosquitto *mosq, void *obj, int rc)
{
    ...
    if ( rc == 0 )
    {
        ...
        me->_logger->logDebug( "MQTT connect failed (" + PithermoUtils::tostring(rc) + "'" );
}
