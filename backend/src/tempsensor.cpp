...
{
    if ( _logger != NULL )
        _logger->logDebug("Sensor reads - t: " + PithermoUtils::ftostring( _temp ) +
                          " h: " + PithermoUtils::ftostring( _humidity ) +
                          " - Last successful read: "  + PithermoUtils::utostring( _timestamp ) );
}
...
bool TempSensor::readSensor()
{
    ...
    _timestamp = PithermoTimer::getTimeEpoc();
#else
